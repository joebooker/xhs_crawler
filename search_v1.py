...
def parse_data(data, keyword, xsec_token,page):
    fieldnames = [
        "笔记id","笔记类型", "笔记链接","笔记标题","笔记正文", "笔记标签",
         "点赞数","评论数", "收藏数", "分享数", "图片链接","发布时间", "用户ip","用户名", "用户id", "用户头像","关键词"
    ]
...
def search(keyword,page,cookies,note_type,sort_type):
    print(page)
    print('----------------------------------------------------------------')
    search_data = {
        "keyword":keyword,
...
def get_data(keyword, start_page, end_page, cookies, img_path, is_download_img,note_type,sort_type):
    logger.info(f"开始获取数据: 关键词={keyword}")
    for page in range(int(start_page),int(end_page)+1):
        logger.info(f"正在获取第 {page} 页")
        data,code = search(keyword,page,cookies,note_type,sort_type)
        # print(data)
        if code == 200:
            ...
            for note in notes:
                ...
                try:
                    ...
                    print(result)
                    note_id,user_id=parse_data(result, keyword, xsec_token,page)
...
if __name__ == '__main__':
    ...
    start_page=1
    end_page=2
    note_type=0#0是综合 1是视频 2是图文
    sort_type='popularity_descending'# popularity_descending,time_descending,general
    cookies_json={
            "a1":'xxxxxx',
            #………………………………
        }
    get_data(keyword,start_page,end_page,cookies_json,img_path,is_download_img,note_type,sort_type)